import type { LoginReply, LoginReq } from '@/api/v1/user/types'
import { defineStore } from 'pinia'
import { computed, ref } from 'vue' // 修复：导入 computed
import { login as apiLogin } from '@/api/v1/user/user'
import { isDoubleTokenMode } from '@/utils'
import { useUserStore } from './user'

interface ISingleTokenRes {
  token: string
  expiresIn: number
}

interface IDoubleTokenRes {
  accessToken: string
...
type IAuthLoginRes = ISingleTokenRes | IDoubleTokenRes
type ILoginForm = LoginReq

function isSingleTokenRes(val: unknown): val is ISingleTokenRes {
  return (
    typeof val === 'object'
    && val !== null
    && 'token' in val
...
function isDoubleTokenRes(val: unknown): val is IDoubleTokenRes {
  return (
    typeof val === 'object'
    && val !== null
    && 'accessToken' in val
    && typeof (val as Record<string, unknown>).accessToken === 'string'
    && 'refreshToken' in val
    && typeof (val as Record<string, unknown>).refreshToken === 'string'
  )
}

function normalizeExpiresIn(expiredAt?: number): number {
  ...
  // 2) `expiredAt` = 剩余有效期（秒）
  if (expiredAt > nowSec) {
    return Math.max(0, expiredAt - nowSec)
  }

  return Math.max(0, expiredAt)
}

function mapLoginReplyToTokenInfo(res: LoginReply | null | undefined): IAuthLoginRes {
  const token = res?.token ?? ''
  const expiresIn = normalizeExpiresIn(res?.expiredAt)
    ...
    uni.login({
      ...
      success: (res) => {
        if (res.code) {
          ...
          reject(new Error('获取微信 code 失败：返回值缺少 code'))
        }
      },
      fail: reject,
    })
...
export const useTokenStore = defineStore(
  ...
  () => {
    // 定义用户信息
    const tokenInfo = ref<IAuthLoginRes>({ ...(tokenInfoState as IAuthLoginRes) })
