...
当任务是 H5、App、小程序页面、移动端路由、登录拦截、移动端本地状态或
app-facing 请求展示时，从本模板开始。涉及平台域名、AppID、真机网络地址
或小程序发布配置时，先确认目标平台和环境，不要把真实账号凭据写入仓库。

当移动端需要新的接口字段、接口路径、校验规则或权限语义时，先修改后端
protobuf/Swagger，再运行 `pnpm api:gen` 同步本模板客户端。只有纯页面文案、
样式、路由标题或本地默认值变化时，才直接留在本模板内完成。
